...
inline int ReplicatedPG::_delete_oid(OpContext *ctx, bool no_whiteout)
{
  ...
  if (pool.info.cache_mode != pg_pool_t::CACHEMODE_NONE && !no_whiteout) {
    ...
    // force modify snapset to ensure it is rewritten after the obj
    // removal above
    ctx->modify_snapset();
...
void ReplicatedPG::make_writeable(OpContext *ctx)
{
  ...
  // use newer snapc?
  const SnapSet* cur_snapset = ctx->get_new_snapset();
  // use newer snapc?
  ...
  if ((ctx->obs->exists && !ctx->obs->oi.is_whiteout()) && // head exist(ed)
    ...
    // clone
    SnapSet *modify_snapset = ctx->modify_snapset();
    cur_snapset = modify_snapset;
    hobject_t coid = soid;
    coid.snap = snapc.seq;
    
    unsigned l;
    for (l=1; l<snapc.snaps.size() && snapc.snaps[l] > modify_snapset->seq; l++) ;
    ...
    if (snap_oi->is_omap())
      ctx->delta_stats.num_objects_omap++;
    ctx->delta_stats.num_object_clones++;
    modify_snapset->clones.push_back(coid.snap);
    modify_snapset->clone_size[coid.snap] = ctx->obs->oi.size;

    // clone_overlap should contain an entry for each clone 
    // (an empty interval_set if there is no overlap)
    modify_snapset->clone_overlap[coid.snap];
    if (ctx->obs->oi.size)
      modify_snapset->clone_overlap[coid.snap].insert(0, ctx->obs->oi.size);
  ...
  if (cur_snapset->clones.size() > 0) {
    ...
    if (is_present_clone(last_clone_oid)) {
      interval_set<uint64_t> newest_overlap = cur_snapset->clone_overlap.rbegin()->second;
      const interval_set<uint64_t>& old_overlap = cur_snapset->clone_overlap.rbegin()->second;
...
void ReplicatedPG::finish_ctx(OpContext *ctx, int log_op_type, bool maintain_ssc)
{
  ...
  if (ctx->is_new_snapset()) {
    dout(20) << "  writing new snapset " << *new_snapset << dendl;
  ...
  if (!maintain_ssc && soid.is_head()) {
    ...
    dout(20) << "  updated ssc " << (void*)ctx->obc->ssc << " snapset to " << *new_snapset << dendl;
  }
